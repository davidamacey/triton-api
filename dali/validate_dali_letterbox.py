...
def calculate_letterbox_affine(orig_w, orig_h, target_size=640):
    ...
    Returns:
        affine_matrix: [2, 3] affine transformation matrix
...
def test_dali_standalone():
    ...
    print(f"  Original size: {orig_w}×{orig_h} (W×H)")
    ...
    assert result.min() >= 0 and result.max() <= 1, f"Values out of range"
...
def test_triton_dali_model():
    ...
    outputs = [
        InferRequestedOutput("preprocessed_images")
    ]
...
def test_ensemble():
    ...
    try:
        if not client.is_model_ready(ensemble_name):
            ...
            return False
        print(f"✓ Ensemble '{ensemble_name}' is ready")
    except Exception as e:
...
def main():
    ...
    print("  ✅ DALI pipeline loads and executes")
    print("  ✅ Affine matrix input working")
    print("  ✅ CPU calculates letterbox parameters")
    print("  ✅ GPU applies transformation")
    print("  ✅ Output format is correct")
    print("  ✅ Full ensemble (DALI + TRT) working")
