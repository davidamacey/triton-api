    ...
    def forward(self, x):
        ...
        if self.class_agnostic == 1:
            num_det, det_boxes, det_scores, det_classes = TRT_EfficientNMS.apply(
                bboxes, scores, self.background_class, self.box_coding,
...
class End2End_TRT(torch.nn.Module):
    ...
    def __init__(self, model, class_agnostic=False, max_obj=100, iou_thres=0.45, score_thres=0.25,
                 mask_resolution=56, pooler_scale=0.25, sampling_ratio=0, max_wh=None, device=None,
                 n_classes=80, is_det_model=True, v10detect=False):
        super().__init__()
        ...
        if is_det_model and not self.v10detect:
            ...
            self.end2end = self.patch_model(class_agnostic, max_obj, iou_thres, score_thres, mask_resolution,
                                            pooler_scale, sampling_ratio, max_wh, device, n_classes)
            self.end2end.eval()
        elif self.v10detect:
...
def export_onnx_trt(self, prefix="ONNX TRT:"):
    ...
    if v10detect:
        self.model = nn.Sequential(
            self.model,
            End2End_TRT(
                self.model, self.args.class_agnostic, self.args.topk_all, self.args.iou_thres,
                self.args.conf_thres, self.args.mask_resolution, self.args.pooler_scale,
                self.args.sampling_ratio, None, self.args.device, labels, is_det_model, v10detect
            )
        )
    else:
...
def apply_end2end_patch():
    ...
    def patched_init(self, cfg=None, overrides=None, _callbacks=None):
        ...
        if not hasattr(self.args, 'topk_all'):
            self.args.topk_all = 100
        if not hasattr(self.args, 'iou_thres'):
            self.args.iou_thres = 0.45
