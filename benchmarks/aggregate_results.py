...
def calculate_speedup(results: Dict[str, Any], baseline: str = "A") -> Dict[str, float]:
    ...
    if baseline not in results:
        ...
        baseline_rps = results[baseline].get("throughput_rps", results[baseline].get("Throughput", 1.0))
...
def generate_summary(results: Dict[str, Any], baseline: str = "A") -> Dict[str, Any]:
    ...
    for track_id, result in sorted_tracks:
        ...
        summary["tracks"][track_id] = {
            ...
            "mean_latency_ms": round(result.get("mean_latency_ms", result.get("MeanLatency", 0)), 2),
            "p50_latency_ms": round(result.get("median_latency_ms", result.get("MedianLatency", 0)), 2),
...
def print_comparison_table(results: Dict[str, Any], speedups: Dict[str, float]):
    ...
    # Header
    print(f"{'Rank':<5} {'Track':<14} {'Throughput':>12} {'Mean (ms)':>11} {'P50 (ms)':>10} "
    ...
    for rank, (track_id, result) in enumerate(sorted_results, 1):
        ...
        print(f"{rank:<5} {track_id:<14} {throughput:>10.1f} rps {mean_lat:>9.2f} {p50_lat:>10.2f} "
...
Examples:
  ...
  %(prog)s --input benchmarks/isolated --baseline B
    ...
    parser.add_argument(
        "--output", "-o",
        help="Output JSON file for summary (optional)"
    )
    parser.add_argument(
        "--csv",
    ...
    parser.add_argument(
        "--baseline", "-b",
        default="A",
        help="Baseline track for speedup calculation (default: A)"
    )
    parser.add_argument(
        "--quiet", "-q",
        action="store_true",
        help="Suppress table output (only generate files)"
    )
